...
- [Réalisation du projet](#réalisation-du-projet)
  ...
  - [Conception PCB avec KiCad](#conception-pcb-avec-kiCad)
...
Pour la simulation, le capteur graphite réel est modélisé dans LTSpice par un circuit électronique dans le figure 4, qui produit une sortie de courant d’environ 150 nA conformément à la valeur réalité (Figure 5). Le sous‑ensemble C3, R5 et le générateur V2 permet de simuler les bruits provenant de l’environnement.
...
Les simulations montrent une tension d’entrée du circuit d’environ 18 mV avant filtrage et amplification (Figure 6). En sortie, le signal est correctement amplifié et filtré, comme illustré sur la figure correspondante 7. Le niveau d’amplification obtenu dépend directement de la valeur choisie pour R2.

### Conception PCB avec KiCad
Après avoir vérifié que le circuit électronique proposé fonctionnait correctement sous LTSpice, nous avons commencé la conception du shield Arduino avec KiCad.

Le shield intègre l’ensemble des éléments nécessaires à notre projet :

- un circuit analogique pour l’amplification et le filtrage du signal du capteur graphite 
- un potentiomètre numérique pour régler le gain du signal 
- un encodeur rotatif permettant de naviguer dans les menus 
- un écran OLED pour afficher les informations et les fonctions du système 
- un module Bluetooth pour la communication avec l’application Android 
- un servomoteur utilisé dans les bancs de test 
- un capteur flex, utilisé comme référence pour comparer les performances avec le capteur graphite
<div align="center">
...
Une fois tous les schémas électroniques réalisés et correctement connectés, nous avons effectué le routage du PCB. Pendant le routage, nous avons pris soin de bien connecter les masses et de tracer les pistes afin d’éviter les interférences. Enfin, un seul via sur la face avant a été utilisé pour faciliter le routage.

La dernière étape est de générer la vue 3D du shield final. Il permet de vérifier la disposition des composants, de s’assurer qu’il n’y a pas de chevauchement et que l’espace disponible est suffisant avant la fabrication du PCB.
...
Après avoir vérifié que le routage du PCB ne présentait aucune erreur, le shield a été fabriqué avec l’aide de Cathy, enseignante du département de Génie Physique.
Une fois le PCB imprimé, nous avons procédé à la mise en place des composants électroniques, puis à leur soudure sur la carte.
<table width="100%">
  <tr>
    <td width="50%" align="center">
...
<div align="center">
  ...
  <em>Il suffit d’appuyer sur l’encodeur pour revenir au menu principal</em>
...
Lorsque l’utilisateur entre dans le menu du servomoteur, l’écran de la figure 15 s’affiche.
En appuyant sur l’encodeur, il est possible de sélectionner un angle : le texte « Choose an angle » passe en majuscules (Figure 16).
Un nouvel appui sur l’encodeur permet d’appliquer l’angle sélectionné au servomoteur, et le mot « APPLY » apparaît en majuscules (Figure 17).
...
* Affichage en temps réel de la variation de la résistance du capteur graphite lorsqu’une déformation est appliquée. La tension ADC est également affichée afin de permettre à l’utilisateur d’ajuster le gain lorsque le signal est saturé ou trop faible. (Figure 19)
* Mesure ponctuelle de la résistance du capteur sélectionné lors de l’appui sur le bouton Measure. L’utilisateur peut saisir la valeur de référence du capteur à l’état non déformé, puis l’application calcule automatiquement le rapport ΔR/R0. (Figure 20)
...
L’application peut être téléchargée via ce code [QR](Application%20Android/QR_code_app.png).
...
<table width="100%">
  <tr>
    <td width="50%" align="center">
      ...
      <br/>
      <em>Figure 27: Répétabilité du capteur graphite – mesures répétées 5 fois</em>
    </td>
    <td width="50%" align="center">
      <img src="Images/Repetabilite_flex_graph1.png" height="250">
      <br/>
      <em>Figure 28: Répétabilité du capteur flex – mesures répétées 5 fois</em>
    </td>
  </tr>
</table>
...
Les figures 27 et 28 montrent la valeur de la résistance mesurée pour chaque angle, avec une courbe correspondant à chaque répétition du test.
Les figures 29 et 30 présentent la valeur moyenne de la résistance pour chaque angle. Les barres d’erreur représentent l’écart‑type calculé à partir des différentes mesures.
...
### Datasheet
La fiche technique du capteur est disponible dans le dossier [Datasheet_capteur_graphite.pdf](Datasheet/Datasheet_capteur_graphite.pdf) du projet.
### Conclusion
Après l’analyse des résultats obtenus lors des bancs de test, nous constatons que le capteur graphite de type 2B présente une bonne sensibilité, proche de celle du capteur flex. De plus, les écarts liés à la répétabilité restent limités et ne sont pas trop importants.

Cependant, malgré ces résultats encourageants, ce capteur graphite simple présente plusieurs inconvénients qui rendent sa commercialisation difficile dans sa forme actuelle.
- Le capteur est fragile. Le support en papier et la couche de graphite peuvent se détériorer après plusieurs utilisations, ce qui réduit la précision des mesures.
- La quantité de graphite déposée sur le papier peut varier à cause de l’environnement, comme la manipulation par l’utilisateur, la température ou l’humidité. Cela entraîne une variation de la résistance initiale au cours du temps.
- Il est également difficile de reproduire exactement le même capteur, car le dépôt manuel du graphite ne permet pas de contrôler précisément la quantité déposée.
- Le contact électrique a une grande influence sur les mesures : selon la position des contacts sur le capteur, les valeurs obtenues peuvent changer, ce qui limite la précision des tests.

À partir de ces limites, nous proposons plusieurs solutions d’amélioration suivantes:
- Utiliser un support plus solide et résistant que le papier, moins sensible aux déformations.
- Ajouter un film de protection pour limiter l’influence de l’environnement sur le graphite.
- Automatiser le dépôt du graphite afin d’assurer une quantité constante de graphite sur papier.
- Encapsuler le capteur avec des contacts électriques fixes pour améliorer la répétabilité et la fiabilité des mesures.

En conclusion, ce projet montre qu’un capteur de déformation à base de graphite peut être une solution simple et peu coûteuse, mais qu’il nécessite encore des améliorations pour être utilisé de manière fiable et envisager une commercialisation.
### Contact
- NGUYEN Phan-Xuan-Anh : pnguye@insa-toulouse.fr
- GAMBOA-SAURY Pablo-David : pgamboa-saur@insa-toulouse.fr